...
def execute_code(symbol):
    ...
    for type_sma in r:
        ...
        if type_sma == r[0]:
            if df['open'].iloc[0] < avg:
                # s += " *** OPEN is under SMA" + str(type_sma) + " *** / "
                open_under_first_sma = True

    evol_close_open = 100 * (df['close'].iloc[0] - df['open'].iloc[0]) / df['open'].iloc[0]

    if at_least_one_is_over and evol_close_open > 1:
        if nb_over == len(r):
            s += "EVOL C-O=" + "{:.2f}".format(evol_close_open) + "%"
            if open_under_first_sma:
                s += " / OPEN < SMA" + str(r[0])
