    ...
    for symbol in symbols:
        ...
        if ohlcv:
            ...
            if percentage_change is not None and percentage_change > percentage_threshold:
                result = f"{symbol} has increased by {percentage_change:.2f}% in the last 10 minutes. Current price: {latest_price:.4f}\n"
                print(result.strip())
    ...
    if results:
        ...
        with open(output_file, 'a') as f:
            for result in results:
                f.write(result)
            f.write('\n')  # Add a new line after each scan batch for readability
    else:
        print("No positive percentage change detected.")
