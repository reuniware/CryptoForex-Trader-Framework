...
#An Idea from Sarah : Detection of Chikou Span (Lagging Span) crossing above the higher BB (Long entry signal) or below the lower one (Short entry signal).
...
def check_timeframe_up(symbol, tf):
    ...
    dframe['bb_bbm'] = indicator_bb.bollinger_mavg()
    dframe['bb_bbh'] = indicator_bb.bollinger_hband()
    dframe['bb_bbl'] = indicator_bb.bollinger_lband()
    #print("BBLOW CHIKOU", dframe['bb_bbl'].iloc[-27])
    #print("BBHIGH CHIKOU", dframe['bb_bbh'].iloc[-27])

    if (dframe['ICH_CS'].iloc[-28] < dframe['bb_bbh'].iloc[-28] and dframe['ICH_CS'].iloc[-27] > dframe['bb_bbh'].iloc[-27]):
        return True
...
def check_timeframe_down(symbol, tf):
    ...
    if (dframe['ICH_CS'].iloc[-28] > dframe['bb_bbl'].iloc[-28] and dframe['ICH_CS'].iloc[-27] < dframe['bb_bbl'].iloc[-27]):
        return True
        ...
        try:
            ...
            if "down" in scantype:
                ...
                if all_tf_ok:
                    ...
                    str_to_log = "SHORT ENTRY SIGNAL for " + symbol + " " + str(array_tf)+ " at " + str(datetime.now()) + " ; Current price = " + str(price_close) + " ; Price evol = " + "{:.4f}".format(price_evol)
                    print(str_to_log)
            ...
            if "up" in scantype:
                ...
                if all_tf_ok:
                    #beep.beep(3)
                    price_evol = get_price_evol(symbol, price_close)
                    str_to_log = "LONG ENTRY SIGNAL for " + symbol + " " + str(array_tf)+ " at " + str(datetime.now()) + " ; Current price = " + str(price_close)  + " ; Price evol = " + "{:.4f}".format(price_evol)
                    print(str_to_log)
                    log_to_results(str_to_log)
...
def main_thread():
    ...
    while True:
        ...
        #décommenter cette ligne pour voir le temps d'obtention concernant tous les assets (ex: si filtrage avec -f *usdt)
