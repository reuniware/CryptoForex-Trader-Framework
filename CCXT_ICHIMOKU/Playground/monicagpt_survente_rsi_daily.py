...
for pair in pairs:
    try:
        rsi = get_rsi(pair)
        if rsi < 30:
            print(f'{pair} est en survente RSI {timeframe} ({rsi:.2f}) à {str(datetime.now()).split(".")[0]}')
        elif rsi > 70:
            print(f'{pair} est en surachat RSI {timeframe} ({rsi:.2f}) à {str(datetime.now()).split(".")[0]}')
        #else:
