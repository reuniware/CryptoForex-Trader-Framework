...
def is_evening_star(candles):
    ...
    # 1. First candle must be a long bullish candle
    # 2. Second candle must be a small-bodied candle (potentially a doji) and it should gap up from the first candle
    # 3. Third candle must be a long bearish candle and it should close below the midpoint of the first candle
...
def analyze_symbol(exchange, symbol, timeframe, output_file):
    try:
        ohlcv = fetch_ohlcv(exchange, symbol, timeframe, 3)
        if not ohlcv:
            return

        # Check if symbol is tradable
        if not fetch_current_price(exchange, symbol):
            return
