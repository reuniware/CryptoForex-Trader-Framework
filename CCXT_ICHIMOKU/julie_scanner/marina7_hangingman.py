...
def scan_and_display_assets(exchange, symbols, timeframe, output_file):
    ...
    for symbol in symbols:
        ...
        if len(ohlcv) >= 2:
            ...
            # Check if the previous candle's date is yesterday
            prev_candle_date = datetime.fromtimestamp(prev_candle[0] / 1000).strftime('%Y-%m-%d')
            if prev_candle_date != yesterday:
                continue
