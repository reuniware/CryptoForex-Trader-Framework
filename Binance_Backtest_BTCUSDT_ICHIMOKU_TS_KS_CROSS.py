...
# STRATEGY :
# WHEN TENKAN SEN CROSSES OVER KIJUN SEN : BUY
# WHEN TENKAN SEN CROSSES UNDER KIJUN SEN : SELL

import pandas as pd
from binance.client import Client
import ta

klinesT = Client().get_historical_klines("BTCUSDT", Client.KLINE_INTERVAL_1HOUR, "01 December 2021")
...
for index, row in df.iterrows():
    if df['ICH_TS'][lastIndex] > df['ICH_KS'][lastIndex] and usdt > 10:
        btc = usdt / df['close'][index]
        btc = btc - 0.0075 * btc
        usdt = 0
        print("Buy BTC at", df['close'][index], '$ the', index)

    if df['ICH_TS'][lastIndex] < df['ICH_KS'][lastIndex] and btc > 0.0001:
