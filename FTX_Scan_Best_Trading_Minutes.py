...
def scan_one(symbol):
    ...
    time.sleep(0.25)    # trying to get rid of UnboundLocalError

    try:
        dframe = pd.DataFrame(data)
    except UnboundLocalError:
        log_to_errors(str(datetime.now()) + " UnboundLocalError for " + symbol)
        print(str(datetime.now()) + " UnboundLocalError for " + symbol)
        time.sleep(1)
    ...
    try:
        ...
        if show_if_cannot_get_all_candlesticks_data:
            log_to_errors(str(datetime.now()) + " cannot get all candlesticks data for " + symbol + " : " + str(i) + " candlesticks have been retrieved instead of " + str(
                nb_candlesticks) + " <=> " + str(round(i, 2)) + " * 1min instead of " + str(nb_candlesticks))
