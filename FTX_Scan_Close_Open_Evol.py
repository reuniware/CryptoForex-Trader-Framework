...
def my_thread(name):
    ...
    while not stop_thread:
        ...
        highest_close_evol = 0
        highest_close_evol_symbol = ""
        ...
        for index, row in df.iterrows():
            ...
            sindex = sindex + 1
            print("(" + str(sindex) + ") " + symbol + " CLOSE/OPEN EVOL = " + str(close_evol) + "    " + str(close0) + " " + str(open0))
            # log_to_results(symbol + " CLOSE EVOL = " + str(close_evol) + "    " + str(close0) + " " + str(close1))

            if close_evol > highest_close_evol:
                highest_close_evol = close_evol
                highest_close_evol_symbol = symbol
                print(str(datetime.now()) + " **** highest /open evol symbol = " + highest_close_evol_symbol + " highest close/open evol = " + str(highest_close_evol))
                log_to_results(str(datetime.now()) + " **** highest close/open evol symbol = " + highest_close_evol_symbol + " highest close/open evol = " + str(highest_close_evol))
