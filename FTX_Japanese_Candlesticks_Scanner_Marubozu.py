...
def execute_code(symbol):
    ...
    for index, row in df.iterrows():
        ...
        low = row['low']
        close = row['close']
        # if openp == low or close == low:
        #     print(timestamp, openp, high, low, close)
        #     log_to_file(symbol_filename, timestamp + " " + str(openp) + " " + str(high) + " " + str(low) + " " + str(close))
        # tolerance = 0.1
        # if low < openp < (low + low / 100 * tolerance):
        ...
        # if low < close < (low + low / 100 * tolerance):
        #     print(timestamp, openp, high, low, close, tolerance, "%")
        #     log_to_file(symbol_filename, timestamp + " " + str(openp) + " " + str(high) + " " + str(low) + " " + str(close) + " " + str(tolerance) + "%")
