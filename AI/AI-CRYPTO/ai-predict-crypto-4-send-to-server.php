...
while True:
  ...
  if (current_prediction != previous_prediction) and create_model is False and discord is True:
      ...
      #webhook.send(">>> Price evol since start = " + "{0:.5f}".format(latest_close_price - initial_close_price))
      try:
        x = requests.get('https://eurodollarbot.000webhostapp.com/index.php/?upload_history=' + str(datetime.now()).split('.')[0] + ";" + "BTC/USDT" + ";" + str("{0:.5f}".format(predicted_diff)) + ";" + str(predicted_price))
        print(str(x))
      except:
