...
history_end_date_str = strday + strmonth + stryear 
interval = Client.KLINE_INTERVAL_4HOUR

data_history_file = crypto + "usdt_data_" + interval + "_01012000_" + history_end_date_str + ".pkl"
...
while not stop:
    ...
    if not (os.path.exists(data_history_file)) or force_download == True:
        ...
        klinesT = Client(tld='us').get_historical_klines(crypto + "USDT", interval, "01 January 2000")
        #klinesT = Client().get_historical_klines("BTCUSDT", interval, "01 January 2000")
        crypto_data = pd.DataFrame(klinesT,
        ...
        crypto_data.to_pickle(data_history_file)
    else:
        print("updating data to merge to existing file with data")
        klinesT2 = Client(tld='us').get_historical_klines(crypto + "USDT", interval, "1 day ago UTC")
        crypto_data2 = pd.DataFrame(klinesT2,
                                        columns=['time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_av',
                                                'trades', 'tb_base_av', 'tb_quote_av', 'ignore'])
        crypto_data2['time'] = pd.to_datetime(crypto_data2['time'], unit='ms')
        print("loading existing file with data")
        crypto_data = pd.read_pickle(data_history_file)
        print("merging downloaded recent data with existing file with data")

        # remove rows from crypto_data2 that are already in crypto_data
        existing_dates = crypto_data['time']
        crypto_data2 = crypto_data2[~crypto_data2['time'].isin(existing_dates)]

        crypto_data = pd.concat([crypto_data, crypto_data2], ignore_index=True, sort=False)
    ...
    model.fit(X_train, y_train, epochs=nb_epochs, batch_size=None, validation_split=0.1, shuffle=False)
    ...
    # Ajustement de la prédiction en fonction du taux d'erreur mape
    if predicted_price > latest_close_price:
      # La prédiction est supérieure au cours actuel, on ajuste à -mape% relativement à predicted_diff
      ajusted_predicted_price = predicted_price - (predicted_diff - predicted_diff/100*mape)
    elif predicted_diff < latest_close_price:
      # La prédiction est inférieure au cours actuel, on ajuste à +mape% relativement à predicted_diff
      ajusted_predicted_price = predicted_price + (predicted_diff + predicted_diff/100*mape)

    print("ajusted predicted price = ", ajusted_predicted_price)
    log_to_results("ajusted predicted price = " + str(ajusted_predicted_price))
    ...
    if avg_ajusted_predict==0:
        avg_ajusted_predict = avg_ajusted_predict + ajusted_predicted_price
    else:
    ...
    #if mape > 1.5: # les meilleurs résultats donnent environs 0.9 pour h1 eurusd
    #    continue
