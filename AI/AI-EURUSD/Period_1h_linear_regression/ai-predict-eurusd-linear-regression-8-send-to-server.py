...
while True:
  ...
  if (current_prediction != previous_prediction) and create_model is False and discord is True:
      ...
      x = requests.get('https://eurodollarbot.000webhostapp.com/?upload_history=' + str(datetime.now()).split('.')[0] + ";" + "EUR/USD : Predicted_diff in points (pips) = " + str(round(predicted_diff * 100000)) + " / Last close = " + "{0:.5f}".format(latest_close_price))
