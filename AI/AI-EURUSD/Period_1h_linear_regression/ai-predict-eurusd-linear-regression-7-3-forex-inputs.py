...
#!zip -r 'modeles_a_trier_eurusd_01062023_0.1172_timestep15.zip' 'modeles_a_trier'
#!cp './modeles_a_trier_eurusd_01062023_0.1172_timestep15.zip' './drive/MyDrive/'
!rm -rf modeles*
#!cp ./drive/MyDrive/modeles_a_trier_eurusd_01062023_0.1172_timestep15.zip ./
#!unzip modeles_a_trier_eurusd_01062023_0.1172_timestep15.zip
...
whole_model_folder_to_load = './modeles_a_trier/20230601200945-whole_model'
show_chart_if_using_existing_model = False

# Définition de la séquence temporelle des pas de temps
#TIME_STEPS = 15 # mape 0.117
TIME_STEPS = 60
...
while True:
  ...
  ohlcv = yf.download('EURUSD=X', start=strStartDate, end=strEndDate, interval='1h') # 60 days backward is the maximum range
  #ohlcv = yf.download('EURUSD=X', start=strStartDate, end=strEndDate, interval='1h') # 730 days backward is the maximum range
  ohlcv.reset_index(inplace=True)

  # Récupération des données de trading de USD/JPY depuis l'API Yahoo Finance
  #ohlcv = yf.download('EURUSD=X', start='2021-06-01', end='2023-05-03', interval='15m')
  ohlcv_usd_jpy = yf.download('EURJPY=X', start=strStartDate, end=strEndDate, interval='1h') # 60 days backward is the maximum range
  #ohlcv = yf.download('EURUSD=X', start=strStartDate, end=strEndDate, interval='1h') # 730 days backward is the maximum range
  ohlcv_usd_jpy.reset_index(inplace=True)

  # Récupération des données de trading de USD/JPY depuis l'API Yahoo Finance
  #ohlcv = yf.download('EURUSD=X', start='2021-06-01', end='2023-05-03', interval='15m')
  ohlcv_usd_cad = yf.download('USDCAD=X', start=strStartDate, end=strEndDate, interval='1h') # 60 days backward is the maximum range
  #ohlcv = yf.download('EURUSD=X', start=strStartDate, end=strEndDate, interval='1h') # 730 days backward is the maximum range
  ohlcv_usd_cad.reset_index(inplace=True)
  ...
  print(ohlcv[-1:])
  print(ohlcv_usd_jpy[-1:])
  print(ohlcv_usd_cad[-1:])
  ...
  usd_jpy_data = pd.DataFrame(ohlcv_usd_jpy, columns=['Datetime', 'Open', 'High', 'Low', 'Close', 'Volume'])
  if (usd_jpy_data.empty):
      print("dataframe is empty")
      sys.exit(-500)
  usd_jpy_data['Datetime'] = pd.to_datetime(usd_jpy_data['Datetime'], unit='ms')

  usd_cad_data = pd.DataFrame(ohlcv_usd_cad, columns=['Datetime', 'Open', 'High', 'Low', 'Close', 'Volume'])
  if (usd_cad_data.empty):
      print("dataframe is empty")
      sys.exit(-500)
  usd_cad_data['Datetime'] = pd.to_datetime(usd_cad_data['Datetime'], unit='ms')


  # Combinaison des données EUR/USD et USD/JPY
  combined_data = pd.merge(eur_usd_data, usd_jpy_data, on='Datetime', suffixes=('_eurusd', '_usdjpy'))
  third_dataframe = usd_cad_data.add_suffix('_usdcad')
  combined_data = pd.concat([combined_data, third_dataframe], axis=1)
  ...
  data = scaler.fit_transform(combined_data[['Close_eurusd', 'Close_usdjpy', 'Close_usdcad']].values)


  # Préparer les données de sortie
  target_col = 'Close_eurusd'
  target = combined_data[target_col].values.reshape(-1, 1)
  ...
  if create_model:
    ...
    #model.compile(optimizer='Adagrad', loss='mse') # 0.189867
    model.compile(optimizer='Adagrad', loss='mae') # 0.184079

    # Entraînement du modèle
    model.fit(X_train, y_train, epochs=300, batch_size=None, validation_split=0.1, shuffle=False)
