...
klinesT = Client().get_historical_klines("BTCUSDT", Client.KLINE_INTERVAL_4HOUR, "01 January 2018")
bitcoin_data = pd.DataFrame(klinesT, columns=['time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_av', 'trades', 'tb_base_av', 'tb_quote_av', 'ignore'])
...
model.fit(X_train, y_train, epochs=30, batch_size=16, validation_split=0.1, shuffle=False)
