...
#!zip -r 'modeles_a_trier_eurusd_05062023_0.1855_timestep60.zip' 'modeles_a_trier'
#!cp './modeles_a_trier_eurusd_05062023_0.1855_timestep60.zip' './drive/MyDrive/'
#!zip -r 'modeles_a_trier_eurusd_09062023_0.1598_timestep60.zip' 'modeles_a_trier'
#!cp './modeles_a_trier_eurusd_09062023_0.1598_timestep60.zip' './drive/MyDrive/'
!rm -rf modeles*
!cp ./drive/MyDrive/modeles_a_trier_eurusd_09062023_0.1598_timestep60.zip ./
!unzip modeles_a_trier_eurusd_09062023_0.1598_timestep60.zip
...
# for now possible assets are EUR/USD, EUR/GBP (and BTC/USDT)
asset = "EUR/GBP"
asset_for_download = asset.replace("/", "")
...
whole_model_folder_to_load = './modeles_a_trier/20230609101808-whole_model'
...
while True:
  ...
  ohlcv = yf.download(asset_for_download + '=X', start=strStartDate, end=strEndDate, interval='1h') # 60 days backward is the maximum range
  ...
  if create_model:
    ...
    model.fit(X_train, y_train, epochs=400, batch_size=None, validation_split=0.1, shuffle=False)
  ...
  if (current_prediction != previous_prediction) and create_model is False and discord is True:
      #webhook.send(str(datetime.now()).split('.')[0] + " > " + asset + " : Predicted_diff in points (pips) = " + str(round(predicted_diff * 100000)) + " ; Last close = " + "{0:.5f}".format(latest_close_price))
      pass
      ...
      try:
        x = requests.get('https://eurodollarbot.000webhostapp.com/?upload_history=' + str(datetime.now()).split('.')[0] + ";" + asset + ";" + str("{0:.5f}".format(predicted_diff)) + ";" + str(predicted_price))
      except:
        print("Upload impossible (2)")
