...
int OnInit()
  {
   ...
   // Check for swing highs and lows on each tick
      ...
      if(high2 > high1 && high2 > high3)
        {
         ...
         double offset = SymbolInfoDouble(_Symbol, SYMBOL_POINT) * 50; // Adjust the offset as needed
         ObjectCreate(_Symbol, arrowName, OBJ_ARROW_DOWN, 0, time, high + offset);
      ...
      if(low2 < low1 && low2 < low3)
        {
         ...
         ObjectCreate(_Symbol, arrowName, OBJ_ARROW_UP, 0, time, low - offset);
         ObjectSetInteger(0, arrowName, OBJPROP_COLOR, clrRed);
        }
     }
  }
//+----------------------By Reuniware / DVASoft (2024) --------------------------------------------+
