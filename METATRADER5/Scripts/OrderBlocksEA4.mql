...
void CheckOrderBlocks()
  {
   ...
   for(int i = 1; i < OrderBlockLookback - 1; i++)
     {
      bool isHighOrderBlock = high[i] > high[i-1] && high[i] > high[i+1];
      bool isLowOrderBlock = low[i] < low[i-1] && low[i] < low[i+1];
      ...
      // Draw middle line if both high and low Order Blocks are detected
      if(isHighOrderBlock && isLowOrderBlock)
        {
         double middlePrice = (high[i] + low[i]) / 2.0;
         DrawMiddleLine(i, middlePrice, time[i]);
        }
     }
  }
   ...
   if(ObjectFind(0, objName) < 0)
     {
      ...
      ObjectSetInteger(0, objName, OBJPROP_COLOR, isHigh ? clrDarkGreen : clrDarkViolet); // Set color based on isHigh
...
//+------------------------------------------------------------------+
//| Tracer une ligne médiane pour un Order Block                     |
//+------------------------------------------------------------------+
void DrawMiddleLine(int barIndex, double price, datetime barTime)
  {
   string objName = "OrderBlockMiddle_" + IntegerToString(barIndex);
   if(ObjectFind(0, objName) < 0)
     {
      ObjectCreate(0, objName, OBJ_HLINE, 0, TimeCurrent(), price);
      ObjectSetInteger(0, objName, OBJPROP_COLOR, clrGray); // Set color to white
      ObjectSetInteger(0, objName, OBJPROP_WIDTH, 1);
      ObjectSetInteger(0, objName, OBJPROP_STYLE, STYLE_DOT);
      ObjectSetInteger(0, objName, OBJPROP_BACK, true); // Draw line behind candles

      // Afficher la date et l'heure de la bougie où la ligne médiane est détectée, ainsi que la valeur de la ligne
      string message = "Middle Line detected at " + TimeToString(barTime, TIME_DATE|TIME_MINUTES) + 
                       ", Price: " + DoubleToString(price, _Digits);
      Print(message);
     }
  }//+------------------------------------------------------------------+
